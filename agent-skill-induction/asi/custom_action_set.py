...
"""
        ...
        # Include MCP tool wrappers in execution context
        # Store the manager instance globally so execution context can access it
        if self.mcp_manager:
            import __main__
            __main__._action_set_mcp_manager = self.mcp_manager
...
"""
    ...
    def _parse_and_include_actions(self, allowed_actions, action_set):
        ...
        for func in allowed_actions:
            ...
            # Handle functions that were already properly processed in agent.py
            if hasattr(func, '_original_tool_name'):
                # These functions are dynamically created and already included in python_includes
                # Just add them to the action set for description purposes
                signature = f"{func.__name__}{inspect.signature(func)}"
                
                # Parse docstring to extract description and examples
                if func.__doc__ and "Examples:" in func.__doc__:
                    description, examples = func.__doc__.split("Examples:", maxsplit=1)
                    description = description.strip()
                    examples = examples.strip()
                else:
                    description = func.__doc__ if func.__doc__ else f"MCP tool: {func.__name__}"
                    examples = f"{func.__name__}()"
                ...
                action_set[func.__name__] = HighLevelAction(
                    ...
                    examples=[examples],
                    function="# Function already included via python_includes",
                    add_code=False
                )
                continue

            if is_mcp_tool:
                # Handle MCP tools specially - create a simple wrapper function
                # Get the original tool name from the function attribute
                original_tool_name = getattr(func, '_original_tool_name', func.__name__.replace('_', '-'))
                
                func_source = f"""
def {func.__name__}(*args, **kwargs):
    '''
    {func.__doc__ if func.__doc__ else 'MCP tool for review analysis'}
    '''
    # MCP tool implementation handled by server
    return globals()['_mcp_tool_wrappers']['{original_tool_name}'](**kwargs)
"""
                self.python_includes += f"""\
{func_source}
...
"""
                ...
                # parse docstring for MCP tools
                if func.__doc__ and "Examples:" in func.__doc__:
                    description, examples = func.__doc__.split("Examples:", maxsplit=1)
                else:
                    description = func.__doc__ if func.__doc__ else f"MCP tool: {func.__name__}"
                    examples = f"{func.__name__}(product_sku='B006H52HBC', keywords=['small', 'narrow'])"
                ...
                action_set[func.__name__] = HighLevelAction(
                    ...
                    add_code=False,  # Don't include code for MCP tools
                )
            else:
                # Handle regular actions as before
