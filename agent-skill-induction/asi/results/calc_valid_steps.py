...
def simplify_thought_with_retry(step: str, max_retries: int = 5) -> str:
    """Simplify the thought part of a step with retry logic for rate limits."""
    ...
    if len(text_parts) > 0:
        ...
        for attempt in range(max_retries):
            try:
                ...
                simplified_thought = response.content[0].text
                simplified_step = f"{simplified_thought}\n"
                break
            except anthropic.RateLimitError as e:
                if attempt == max_retries - 1:
                    print(f"Rate limit exceeded after {max_retries} attempts. Skipping this step.")
                    simplified_step = "Rate limit exceeded - step skipped\n"
                    break
                ...
                if attempt == max_retries - 1:
                    simplified_step = f"Error occurred - step skipped: {str(e)}\n"
                    break
...
def simplify_thought(step: str) -> str:
    ...
    return simplify_thought_with_retry(step)
